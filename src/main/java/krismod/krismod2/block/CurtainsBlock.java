...
public class CurtainsBlock extends FacingBlock implements BlockEntityProvider {
    public CurtainsBlock() { super(Settings.create().nonOpaque());}

    @Nullable
    @Override
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return BlockEntityRegistry.CURTAINS_TEST.instantiate(pos, state);
    }
    ...
    public void appendTooltip(ItemStack stack, BlockView blockGetter, List<Text> tooltip, TooltipContext tooltipFlag) {
        tooltip.add(Text.translatable("block." + Krismod2.MOD_ID + ".curtainsblock.tooltip"));
